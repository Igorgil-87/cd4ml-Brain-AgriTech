...
import cd4ml.problems.insumo.splitting as splitting
import cd4ml.problems.insumo.readers.stream_data as data_streamer
import cd4ml.problems.insumo.download_data.download_data as dd
import logging
...
class Problem(ProblemBase):
    ...
    def get_feature_set_constructor(feature_set_name):
        if feature_set_name == "default":
            from cd4ml.problems.insumo.features.feature_sets.default import feature_set as fs
            return fs.get_feature_set
        elif feature_set_name == "original":
            from cd4ml.problems.insumo.features.feature_sets.original import feature_set as fs
            return fs.get_feature_set
        else:
            raise ValueError("feature_set_name '{}' is invalid. Please check your insumo configuration"
