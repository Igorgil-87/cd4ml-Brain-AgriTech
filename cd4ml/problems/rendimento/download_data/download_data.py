...
# Função para carregar dados do banco de forma segura
def query_db_safe(table_name, chunksize=1000):
    try:
        # Usar o gerador para criar o DataFrame completo
        data_gen = pd.read_sql(f"SELECT * FROM {table_name}", con=engine, chunksize=chunksize)
        data = pd.concat(data_gen, ignore_index=True)  # Combina os chunks em um único DataFrame
...
def create_rendimento_raw():
    ...
    try:
        ...
        if milho is None or arroz is None or ranking_valores is None:
            print("Erro: Uma ou mais tabelas necessárias não foram carregadas. Abortando criação de rendimento_raw.")
            return
...
def download(problem_name=None):
    ...
    # Renomear colunas e processar dados
    ranking_valores.rename(columns={"produto": "Cultura", "valor": "Valor da Produção Total"}, inplace=True)
    clean_and_convert(ranking_valores, "Valor da Produção Total")
    ...
    if milho_transformado is None or arroz_transformado is None:
        print("Erro: Uma ou mais tabelas de dados transformados não foram carregadas. Abortando download.")
        return
