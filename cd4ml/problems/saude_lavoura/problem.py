...
import cd4ml.problems.saude_lavoura.readers.stream_data as stream_data
from cd4ml.problems.saude_lavoura.download_data.download_data import download
...
class Problem(ProblemBase):
    ...
    def get_feature_set_constructor(feature_set_name):
        if feature_set_name == 'default':
            import cd4ml.problems.saude_lavoura.features.feature_sets.default.feature_set as default_features
            return default_features.get_feature_set
        else:
            raise ValueError(f"Featureset name {feature_set_name} is not valid")
